...
void menuBar()
{
	iShowImage(0, 0, screenWidth, screenHeight, iLoadImage("Image\\homepage.bmp"));
	for (int i = 0; i < 5; i++)
	{
...
void shinzo_Movement()
{
	if (shinzoVisible){
		if (jump){
			if (jumpUp){
				iShowBMP2(shinzoX, shinzoY + shinzoJump, shinzo_jump[0], 0);
			}
			else{
				iShowBMP2(shinzoX, shinzoY + shinzoJump, shinzo_jump[1], 0);
			}
		}
		if (standfront)
		{
			runForward = runBackward = false;
			iShowBMP2(shinzoX, shinzoY, shinzo_stand[stand_index], 0);
		}
		if (standrev)
		{
			runForward = runBackward = false;
			iShowBMP2(shinzoX, shinzoY, stand_rev[stand_index], 0);
		}
		if (firing&&front)
		{
		...
		if (runForward)
		{
			iShowBMP2(shinzoX, shinzoY, shinzo_run[runIndex], 0);
			stand_count++;
			if (stand_count >= 20)
			{
				stand_count = 0;
		...
		if (runBackward)
		{
			iShowBMP2(shinzoX, shinzoY, run_rev[runIndex], 0);
			stand_count++;
			if (stand_count >= 20)
			{
				stand_count = 0;
...
void shinzo_Movement_lvl2()
{
	if (shinzoVisible2)
	{
		if (jump)
		{
			if (jumpUp)
			{
				iShowBMP2(shinzoX1, shinzoY1 + shinzoJump, shinzo_jump[0], 0);
			}
			else
			{
				iShowBMP2(shinzoX1, shinzoY1 + shinzoJump, shinzo_jump[1], 0);
			}
		}
		if (standfront)
		{
			runForward = runBackward = false;
			iShowBMP2(shinzoX1, shinzoY1, shinzo_stand[stand_index], 0);
		}
		if (standrev)
		{
			runForward = runBackward = false;
			iShowBMP2(shinzoX1, shinzoY1, stand_rev[stand_index], 0);
		}
		if (firing&&front)
		{
		...
		if (runForward)
		{
			iShowBMP2(shinzoX1, shinzoY1, shinzo_run[runIndex], 0);
			stand_count++;
			if (stand_count >= 20)
			{
				stand_count = 0;
		...
		if (runBackward)
		{
			iShowBMP2(shinzoX1, shinzoY1, run_rev[runIndex], 0);
			stand_count++;
			if (stand_count >= 20)
			{
				stand_count = 0;
...
void shinzo_Movement_lvl3()
{
	if (shinzoVisible3){
		if (jump){
			if (jumpUp){
				iShowBMP2(shinzoX2, shinzoY2 + shinzoJump, shinzo_jump[0], 0);
			}
			else{
				iShowBMP2(shinzoX2, shinzoY2 + shinzoJump, shinzo_jump[1], 0);
			}
		}
		if (standfront)
		{
			runForward = runBackward = false;
			iShowBMP2(shinzoX2, shinzoY2, shinzo_stand[stand_index], 0);
		}
		if (standrev)
		{
			runForward = runBackward = false;
			iShowBMP2(shinzoX2, shinzoY2, stand_rev[stand_index], 0);
		}
		if (firing&&front)
		{
		...
		if (runForward)
		{
			iShowBMP2(shinzoX2, shinzoY2, shinzo_run[runIndex], 0);
			stand_count++;
			if (stand_count >= 20)
			{
				stand_count = 0;
		...
		if (runBackward)
		{
			iShowBMP2(shinzoX2, shinzoY2, run_rev[runIndex], 0);
			stand_count++;
			if (stand_count >= 20)
			{
				stand_count = 0;
...
void collisionCheck()
{
	if (!pause)
	{
		if (level == 1 && level1characters)
		{
			for (int i = 0; i < monsterNumber; i++)
			{
				if (monster_lvl1[i].monster_visible&&shinzoVisible)
				{
					if ((shinzoX >= monster_lvl1[i].x && shinzoX <= monster_lvl1[i].x + 100) && (shinzoY + 20 >= monster_lvl1[i].y && shinzoY <= monster_lvl1[i].y + 120)){
						life_index++;
						if (life_index >= 5)
						{
							life_index = 5;
		...
		if (level == 2 && level2characters)
		{
			for (int i = 0; i < monsterNumber2; i++)
			{
				if (monster_lvl2[i].monster_visible&&shinzoVisible2)
				{
					if ((shinzoX1 + 30 >= monster_lvl2[i].x && shinzoX1 <= monster_lvl2[i].x + 100) && (shinzoY1 + 20 >= monster_lvl2[i].y && shinzoY1 <= monster_lvl2[i].y + 120)){
						life_index++;
						if (life_index >= 5)
						{
							life_index = 5;
			...
			for (int i = 0; i < obstacleNumber; i++)
			{
				if (obstacle[i].obstacle_visible&&shinzoVisible2)
				{
					if ((shinzoX1 + 30 >= obstacle[i].x && shinzoX1 <= obstacle[i].x + 74) && (shinzoY1 + 20 >= obstacle[i].y && shinzoY1 + shinzoJump <= obstacle[i].y + 88)){
						life_index++;
						if (life_index >= 5)
						{
							life_index = 5;
...
void bosscollisioncheck()
{
	if (!pause){
		if (level == 3 && level3characters){
			if (bossfiring){
				if ((shinzoX2 <= bossfireX + 27 && shinzoX2 + 77 >= bossfireX) && (shinzoY2 + shinzoJump <= bossfireY + 19)){
					life_index++;
			...
			if (level == 1){
				for (int i = 0; i < monsterNumber; i++){
					if (monster_lvl1[i].monster_visible){
						if ((fireX + 40 >= monster_lvl1[i].x && fireX <= monster_lvl1[i].x + 100) && (fireY <= monster_lvl1[i].y + 120 && fireY + 35 >= monster_lvl1[i].y)){
							monster_lvl1[i].monster_visible = false;
							firing = false;
							fireX = shinzoX + 10000000000000000000;
							enemyKilled++;
							killScore += 5;
						}
					}
				...
				for (int i = 0; i < monsterNumber2; i++){
					if (monster_lvl2[i].monster_visible){
						if ((fireX + 40 >= monster_lvl2[i].x && fireX <= monster_lvl2[i].x + 100) && (fireY <= monster_lvl2[i].y + 120 && fireY + 35 >= monster_lvl2[i].y)){
							monster_lvl2[i].monster_visible = false;
							firing = false;
							fireX = shinzoX1 + 10000000000000000000;
							enemyKilled++;
							killScore += 5;
						}
					}
				...
				if (bossvisible){
					if ((fireX >= bossX && fireX <= bossX + 120) && (fireY <= bossY + 124 && fireY >= bossY)){
						boss_index++;
						firing = false;
						fireX = shinzoX2 + 10000000000000000000;
						killScore += 5;
...
void iDraw()
{
	iClear();
	if (gameState == -3)
		iShowImage(0, 0, 1200, 600, iLoadImage("Image\\firstpage2.png"));
	...
	else if (gameState == 0 && level == 1)
	{
		...
		iShowImage(0, 0, screenWidth, screenHeight, iLoadImage("Image\\About1.png"));
	else if (gameState == 2)
	{
		iShowImage(0, 0, screenWidth, screenHeight, iLoadImage("Image\\highscore1.png"));
		showscore();
	}
	else if (gameState == 3)
		iShowImage(0, 0, screenWidth, screenHeight, iLoadImage("Image\\Credit1.png"));
	else if (gameState == 4)
		exit(0);
	else if (gameOver == true){
		iShowImage(0, 0, screenWidth, screenHeight, iLoadImage("Image\\gameover1.png"));
		resetGame();
...
void level_end_logic()
{
	...
	else if (level == 3 && bossvisible == false && shinzoVisible3 == true)
		chestv = true;
}
...
{
	for (int i = 0; i < obstacleNumber; i++){
		obstacle[i].x = screenWidth + rand() % 40000;
		obstacle[i].y = 90;
		obstacle[i].obstacle_visible = true;
	}
}
...
void iKeyboard(unsigned char key)
{
	...
	else if (key == 'm' || key == 'M')
	{
		if (musicOn)
		{
			...
			musicOn = true;
			PlaySound("music\\bgm1.wav", NULL, SND_LOOP | SND_ASYNC);
	...
	else if ((key == 'd' || key == 'D'))
	{
		if (!pause)
		{
			...
			runBackward = false;
			if (jump) 
				runBackward = runForward = false;
			if (level == 1)
			{
				shinzoX += 5;
				if (shinzoX >= screenWidth)
					shinzoX = -5;
			}
			else if (level == 2)
			{
				shinzoX1 += 5;
				if (shinzoX1 >= screenWidth)
					shinzoX1 = -5;
			}
			else if (level == 3)
			{
				shinzoX2 += 5;
				if (shinzoX2 >= screenWidth)
					shinzoX2 = -5;
			}
			runIndex++;
			if (runIndex >= 5)
...
int main()
{
	...
	t1 = iSetTimer(135, changeBackground);
	iSetTimer(50, change);
